...
#include "ui_threecamsviderostitching.h"

threeCamsVideroStitching::threeCamsVideroStitching(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::threeCamsVideroStitching)
{
    ui->setupUi(this);
}

threeCamsVideroStitching::~threeCamsVideroStitching()
{
    delete ui;
}
...
void threeCamsVideroStitching::CalcFourCorner1(Mat image1)
{
    Mat Htmp;
    H12.copyTo(Htmp);
    CvMat cvH=Htmp;
...
void threeCamsVideroStitching::CalcFourCorner2(Mat image1)
{
    Mat Htmp;
    H23.copyTo(Htmp);
    CvMat cvH=Htmp;
...
/*
void threeCamsVideroStitching::CalcFourCorner3(Mat image1)
{
    Mat Htmp;
    H34.copyTo(Htmp);
    CvMat cvH=Htmp;
...
Mat threeCamsVideroStitching::findHMatrix(Mat image1,Mat image2)
{
    //
    Mat H;
...
Mat threeCamsVideroStitching::imageMix1(Mat image1,Mat image2)
{
    Mat result;
    //todo
...
Mat threeCamsVideroStitching::imageMix2(Mat image1,Mat image2)
{
    Mat result;
    //todo
...
/*
Mat threeCamsVideroStitching::imageMix3(Mat image1,Mat image2)
{
    Mat result;
    //todo
...
void threeCamsVideroStitching::on_HMatrixCalcBtn_clicked()
{
    ...
    Mat videoImageSrc1,videoImageSrc2,videoImageSrc3;
    ...
    Mat result=imageMix2(result12,image3);

    imshow("result",result);

}

void threeCamsVideroStitching::on_videoStitchingBtn_clicked()
{
    VideoCapture cap1(0);
    VideoCapture cap2(1);
    VideoCapture cap3(2);
    ...
    VideoWriter writerResult("result.avi", CV_FOURCC('M', 'J', 'P', 'G'), 25.0, Size(MIN(rightTop2.x,rightBottom2.x), image3.rows));
    ...
    while(1) //一直循环
    {
        ...
        result=imageMix2(result12,image3);
        imshow("result",result);
